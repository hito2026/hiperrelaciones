...
function renderRecords(){
  ...
  const sorted=[...groups].sort(([a],[b])=>a.localeCompare(b,"es")),headers=[["date_utc","Fecha/hora"],["person","Personal"],["counterparts","Contraparte(s)"],["source","Fuente"],["kind","Tipo"],["model","Modelo"],["res_id","ID/SHA"],["title","Tarea / TK / título"],["change","Anterior → nuevo"],["hours","Horas"],["project","Proyecto"],["created_by","Autor / clasificación"],["quality","Q"],["text","Evidencia / texto"],["coverage","Cobertura"]];
...
function renderRecord(record){
  ...
  const kinds=record.kinds.map(kind=>kindLabel[kind]).join(", "),changes=record.subevents.filter(item=>item.field).map(item=>`${item.old||"∅"} → ${item.new||"∅"}`).join("; ")||"—",projects=[...new Set(record.subevents.map(item=>item.project).filter(Boolean))].join(", ")||"—";
  const authors=[...new Set(record.subevents.filter(item=>item.kind==="timesheet").map(item=>item.entry_class==="own_entry"?"Carga propia":`Carga por ${item.created_by}`))].join(", ")||"—",qualities=record.subevents.filter(item=>item.quality!==undefined),quality=qualities.length?Math.round(100*qualities.reduce((sum,item)=>sum+item.quality,0)/qualities.length)+"/100":"—";
  const evidence=record.batch?`${record.source_event_count} tareas distintas creadas en una operación masiva`:record.subevents.map(item=>item.text).filter(Boolean).join(" · ");
  ...
  return `<tr><td>${esc(recordLocalDate(record))}</td><td>${esc(record.person)}</td><td>${esc(counterpart)}</td><td>${esc(recordSource(record))}</td><td>${esc(kinds)}</td><td>${esc(record.model||"—")}</td><td>${idCell}</td><td>${title}</td><td>${esc(changes)}</td><td>${record.hours||"—"}</td><td>${esc(projects)}</td><td>${esc(authors)}</td><td>${quality}</td><td class="evidence-cell"><button type="button" data-record-key="${esc(record.key)}" title="${esc(evidence)}">${esc(evidence||"Ver cronología")}</button></td><td>${record.source_event_count} fuente${record.derived_event_count?` + ${record.derived_event_count} derivada(s)`:""}</td></tr>`;
}
