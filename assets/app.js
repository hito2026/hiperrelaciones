...
function renderRecords(){
  ...
  const sorted=[...groups].sort(([a],[b])=>a.localeCompare(b,"es")),headers=[["date_utc","Fecha/hora"],["person","Personal"],["counterparts","Contraparte(s)"],["source","Fuente"],["kind","Tipo"],["model","Modelo"],["res_id","ID/SHA"],["title","Tarea / TK / título"],["project","Proyecto"],["hours","Horas registradas"],["text","Descripción literal"]];
...
function renderRecord(record){
  ...
  const hours=record.hours>0?`${record.hours.toFixed(2)} h`:"—";
  return `<tr><td>${esc(recordLocalDate(record))}</td><td>${esc(record.person)}</td><td>${esc(counterpart)}</td><td>${esc(recordSource(record))}</td><td>${esc(kinds)}</td><td>${esc(record.model||"—")}</td><td>${idCell}</td><td>${title}</td><td>${esc(projects)}</td><td>${esc(hours)}</td><td class="evidence-cell"><button type="button" data-record-key="${esc(record.key)}" title="${esc(evidence)}">${esc(evidence)}</button></td></tr>`;
}
